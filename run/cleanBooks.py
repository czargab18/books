...
def limpar_recursivo(base_dir, limpador):
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file.lower().endswith(".html"):
                caminho = os.path.join(root, file)
                limpar_arquivo_html(caminho, limpador)
...
if __name__ == "__main__":
    ...
    if args.base_dir:
        limpar_recursivo(args.base_dir, limpador)
    elif args.file:
        limpar_arquivo_html(args.file, limpador)
    else:
        print(
