...
# fórmulas LaTeX
Este documento utiliza fórmulas LaTeX que estão salvas em arquivos separados e são importadas para cá quando necessário. Essa abordagem torna o código do documento mais limpo, facilita a reutilização das fórmulas em diferentes partes do projeto e simplifica a manutenção e atualização das expressões matemáticas. Assim, é possível centralizar as definições e garantir consistência em toda a documentação.

## Exemplo de Fórmulas Avançadas

1. Fórmula com parâmetro:
   $\formulaComParametro{5}$

2. Fórmula condicional:
   $\formulaCondicional{A}$

   $\formulaCondicional{B}$

   $\formulaCondicional{C}$

3. Fórmula avançada com parâmetros:
   $\formulaAvancada[Opcional]{Obrigatório}$
