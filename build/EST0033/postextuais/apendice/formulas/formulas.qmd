...
Dados não agrupados: Para dados ordenados: se $n$ ímpar, $Md = x_{(\frac{n+1}{2})}$; se $n$ par, $Md = \frac{x_{(\frac{n}{2})} + x_{(\frac{n}{2}+1)}}{2}$
...
Dados agrupados: $$Mo = L + \frac{(f_1 - f_0)}{(2f_1 - f_0 - f_2)} \cdot h$$ onde: $L$ = limite inferior da classe modal, $f_1$ = frequência da classe modal, $f_0$ = frequência da classe anterior, $f_2$ = frequência da classe posterior, $h$ = amplitude da classe.
